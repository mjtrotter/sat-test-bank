...
async def test_batch_tag_dry_run(seeded_session):
    results = await batch_tag(seeded_session, dry_run=True, skip_tagged=False)
...
async def test_batch_tag_no_dry_run(seeded_session):
    results = await batch_tag(seeded_session, dry_run=False, skip_tagged=False)
    assert len(results) > 0
...
async def test_batch_tag_skip_tagged(seeded_session):
    ...
    results = await batch_tag(seeded_session, dry_run=False, skip_tagged=True)
