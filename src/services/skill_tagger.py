...
async def tag_problem(session: AsyncSession, problem: Problem) -> list[TagCandidate]:
    """Find valid candidates for a single problem."""
    # 1. Classify domain
    domains = classify_domain(problem.problem_text)
    if not domains:
        return []
    ...
    # 2. Estimate level
    level = estimate_level(problem.contest_family, problem.contest_level, problem.problem_number)

    # 3. Match nodes
    candidates = await match_skill_nodes(session, primary_domain.domain, level, primary_domain.score)

    return candidates


async def batch_tag(session: AsyncSession, dry_run: bool = False, skip_tagged: bool = True) -> list[ProblemTagResult]:
    """Batch tag problems."""
    query = select(Problem)
    if skip_tagged:
        # Get problem IDs that already have tags
        subq = select(ProblemSkillTag.problem_id)
        query = query.where(Problem.id.not_in(subq))
    ...
    for problem in problems:
        ...
        if candidates:
            results.append(ProblemTagResult(problem_id=problem.id, candidates=candidates))

            if not dry_run:
                for cand in candidates:
                    tag = ProblemSkillTag(
                        problem_id=problem.id,
                        skill_node_id=cand.skill_node_id,
